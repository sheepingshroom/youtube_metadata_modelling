...
OUTPUT_PATH = {
    "train_unbal": os.path.join(DATA_FOLDER,"training_final.pkl"),
    "val_unbal": os.path.join(DATA_FOLDER,"validation_final.pkl"),
    "test_unbal": os.path.join(DATA_FOLDER,"testing_final.pkl"),
    ...
    "train_universe": os.path.join(DATA_FOLDER,"training_universe7.pkl"),
    "val_universe": os.path.join(DATA_FOLDER,"validation_universe7.pkl"),
    "test_universe": os.path.join(DATA_FOLDER,"testing_universe7.pkl"),
    "tokenizer_universe": os.path.join(DATA_FOLDER,"tokenizer_final.pkl")
}
...
def generate_dataset(df):
    ...
    logger.info(f"Length before trimming counter: {len(counter)}") 
    ...
    logger.info(f"Length after trimming counter: {len(counter)}") 
    ...
    #keras tokenization
    tokenizer = Tokenizer(len(counter), oov_token=True)
    ...
    logger.info(f"Text: {split['train']['title'][10:15]}") 
    logger.info(f"Numeric tokens: {sequences['train'][10:15]}") 
...
if __name__ == "__main__":
    test_index = int(df["unbal"].shape[0] * (1-TEST_RATIO))
    df["unbal"][test_index:].to_csv(OUTPUT_PATH["test_unbal"], chunksize=1000000)
    output = generate_dataset(df["unbal"][:test_index])
    output["train"].to_pickle(OUTPUT_PATH["train_unbal"])
    print("saved training set")
    output["val"].to_pickle(OUTPUT_PATH["val_unbal"])
    print("saved val set")
